  ...
  class GameScene extends Phaser.Scene {
    ...
    _flap() {
      ...
      if (!this.isStarted) {
        ...
        this.pipeTimer = PIPE_SPAWN_INTERVAL - 800; // first pipe sooner
        SoundEngine.swoosh();
      }
    ...
    _spawnPipePair() {
      ...
      topPipe.body.setSize(PIPE_WIDTH, topHeight);
      topPipe.body.setOffset(0, 0);
      this.pipes.add(topPipe);

      // Top pipe cap collision area
      const topCap = this.add.rectangle(W + 10, topHeight, PIPE_WIDTH + 12, 22);
      this.physics.add.existing(topCap, false);
      ...
      bottomPipe.body.setSize(PIPE_WIDTH, bottomHeight);
      bottomPipe.body.setOffset(0, 0);
      this.pipes.add(bottomPipe);

      // Bottom pipe cap collision area
      const bottomCap = this.add.rectangle(W + 10, bottomY, PIPE_WIDTH + 12, 22);
      this.physics.add.existing(bottomCap, false);
